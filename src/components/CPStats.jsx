...
const StatCard = ({ title, data, color, index }) => (
  <motion.a 
    href={data.url} 
    target="_blank" 
    rel="noopener noreferrer"
...
const BigStatCard = ({ icon: Icon, title, value, suffix, description, index }) => (
  <motion.div 
    className="group relative bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-2xl border border-slate-700 hover:border-cyan-500 transition-all duration-300 hover:-translate-y-2 overflow-hidden"
    initial={{ opacity: 0, y: 30 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true }}
    transition={{ delay: index * 0.1 }}
  ...
  >
    ...
    />
    <motion.div 
      className="absolute top-4 right-4 w-20 h-20 bg-cyan-500/10 rounded-full blur-2xl group-hover:bg-cyan-500/20 transition-all duration-300"
    ...
    <div className="relative">
      <div className="flex items-center gap-3 mb-4">
        <motion.div 
          className="p-3 bg-slate-700 rounded-xl group-hover:bg-cyan-500/20 transition-colors"
      ...
      <div className="flex items-baseline gap-2">
        <motion.span 
          className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 group-hover:from-cyan-300 group-hover:to-blue-400 transition-all duration-300"
    ...
    <motion.div 
      className="absolute bottom-0 left-0 w-0 h-1 bg-gradient-to-r from-cyan-400 to-blue-500 group-hover:w-full transition-all duration-500"
      initial={{ width: 0 }}
    ...
    >
      <motion.div 
        className="absolute top-0 right-0 w-8 h-8 bg-cyan-500/20 rounded-bl-full transform translate-x-2 -translate-y-2 group-hover:bg-cyan-500/30 transition-colors"
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto scroll-mt-20">
        ...
        >
          <StatCard title="Codeforces" data={profile.cpStats.codeforces} color="text-green-400" index={0} />
          <StatCard title="CodeChef" data={profile.cpStats.codechef} color="text-yellow-400" index={1} />
          <StatCard title="AtCoder" data={profile.cpStats.atcoder} color="text-blue-400" index={2} />
